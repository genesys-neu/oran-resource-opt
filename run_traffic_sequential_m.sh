...
user_tuples[8]="1 0 0"
user_tuples[9]="1 0 0"
user_tuples[10]="1 0 0"
user_tuples[11]="1 0 0"
user_tuples[12]="1 0 0"
user_tuples[13]="1 0 0"
user_tuples[14]="1 0 0"
user_tuples[15]="1 0 0"
user_tuples[16]="1 0 0"
user_tuples[17]="1 0 0"
user_tuples[18]="1 0 0"
user_tuples[19]="1 0 0"
user_tuples[20]="1 0 0"
user_tuples[21]="1 0 0"
user_tuples[22]="1 0 0"
user_tuples[23]="1 0 0"
user_tuples[24]="1 0 0"
user_tuples[25]="1 0 0"
user_tuples[26]="1 0 0"
user_tuples[27]="1 0 0"
user_tuples[28]="1 0 0"
user_tuples[29]="1 0 0"
user_tuples[30]="1 0 0"
user_tuples[31]="1 0 0"
user_tuples[32]="1 0 0"
user_tuples[33]="1 0 0"
user_tuples[34]="1 0 0"
user_tuples[35]="1 0 0"
user_tuples[36]="1 0 0"
user_tuples[37]="1 0 0"
user_tuples[38]="1 0 0"
user_tuples[39]="1 0 0"
user_tuples[40]="1 0 0"
user_tuples[41]="1 0 0"
user_tuples[42]="1 0 0"
user_tuples[43]="1 0 0"
user_tuples[44]="1 0 0"
user_tuples[45]="1 0 0"
user_tuples[46]="1 0 0"
user_tuples[47]="1 0 0"
user_tuples[48]="1 0 0"
user_tuples[49]="1 0 0"
user_tuples[50]="1 0 0"
...
traffic_gen() {
    ...
    for entry in $slice_mapping; do
        ...
        local files=($(ls "./raw" | grep -i "$slice_type" | sort))

        if [ ${#files[@]} -eq 0 ]; then
            echo "No files found for slice type: $slice_type"
            continue
        fi

        # Get the last used index and update it
        local file_index=${file_index_map[$slice_type]:-0}
        local file="${files[$file_index]}"

        # Update index for the next call, cycling back to 0 when reaching the end
        file_index_map[$slice_type]=$(( (file_index + 1) % ${#files[@]} ))

        local ip=$((ue_id + 1))

        echo "Starting TGEN for $slice_type on UE: $ue_id"
        echo "Using trace: ${file}"
        echo "Using IP: 172.16.0.${ip}"

        sshpass -p 'scope' ssh $gnb "cd TRACTOR && timeout 160 python traffic_gen.py --eNB -f ./raw/${file} --ip 172.16.0.${ip} -eNBp ${eNB_PORT} -UEp ${UE_PORT}" &
        sleep 2

        local line=$(sed -n "${ue_id}p" "$config_file")
        sshpass -p 'scope' ssh $line "cd TRACTOR && timeout 158 python traffic_gen.py -f ./raw/${file} -eNBp ${eNB_PORT} -UEp ${UE_PORT}" &

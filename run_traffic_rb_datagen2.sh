...
while [ $experiments -le 10 ]; do
  ...
  if [ $experiments -eq 1 ]; then
    ...
    echo "Using default slice-users"
    sleep 10
    # "slice-users": "{0: [11], 1: [2, 3], 2:[4, 5, 6, 7, 8, 9, 10]}"
    user_tuple="1 2 6"
    ...
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [11], 1: [2, 3, 4], 2:[5, 6, 7, 8, 9, 10]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="1 3 5"
    sshpass -p "scope" ssh $gnb "
      sed -i '8s/::.*/::2/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '9s/::.*/::2/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [10, 11], 1: [2, 3, 4], 2:[5, 6, 7, 8, 9]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="2 3 4"
    sshpass -p "scope" ssh $gnb "
      sed -i '11s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '2s/::.*/::1/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [6, 7, 11], 1: [2, 8, 9], 2:[3, 4, 5, 10]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="3 2 3"
    sshpass -p "scope" ssh $gnb "
    ...
    for num in $(seq 2 11); do
      ...
      line=$(sed -n "${num}p" "$1")
      echo "Starting TGEN for SRN: $line"
      if [ "$num" -eq 2 ] || [ "$num" -eq 8 ]; then
        random_file=$(ls "./raw" | grep -i "urll" | shuf -n 1)
        # start URLLC (random trace)
      elif [ $num -ge 3 ] && [ $num -le 5 ]; then
        random_file=$(ls "./raw" | grep -i "embb" | shuf -n 1)
        # start eMBB (random trace)
      elif [ "$num" -eq 6 ] || [ "$num" -eq 7 ] || [ "$num" -eq 11 ]; then
        random_file=$(ls "./raw" | grep -i "mmtc" | shuf -n 1)
        # start mmtc (random trace)
      fi
      ip=$((num + 1))
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [6, 7, 11], 1: [2, 8, 9, 10], 2:[3, 4, 5]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="3 3 2"
    sshpass -p "scope" ssh $gnb "
      sed -i '6s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '7s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [6], 1: [2, 8, 9, 10], 2:[3, 4, 5, 7, 11]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="1 4 4"
    sshpass -p "scope" ssh $gnb "
      sed -i '6s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '7s/::.*/::2/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [7, 8], 1: [2, 9, 10, 11], 2:[3, 4, 5, 6]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="2 4 3"
    sshpass -p "scope" ssh $gnb "
      sed -i '7s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    for num in $(seq 2 11); do
      ...
      line=$(sed -n "${num}p" "$1")
      echo "Starting TGEN for SRN: $line"
      if [ "$num" -eq 2 ] || [ "$num" -eq 9 ] || [ "$num" -eq 10 ] || [ "$num" -eq 11 ]; then
        random_file=$(ls "./raw" | grep -i "urll" | shuf -n 1)
        # start URLLC (random trace)
      elif [ $num -ge 3 ] && [ $num -le 5 ]; then
        random_file=$(ls "./raw" | grep -i "embb" | shuf -n 1)
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [7, 8, 9], 1: [2, 3, 10], 2:[4, 5, 6, 11]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="3 3 3"
    sshpass -p "scope" ssh $gnb "
      sed -i '7s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '8s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
    ...
    for num in $(seq 2 11); do
      ...
      line=$(sed -n "${num}p" "$1")
      echo "Starting TGEN for SRN: $line"
      if [ $num -eq 2 ] || [ $num -eq 3 ] || [ $num -eq 10 ]; then
        random_file=$(ls "./raw" | grep -i "urll" | shuf -n 1)
        # start URLLC (random trace)
    ...
    # update slice-users
    echo "Updating slice-users"
    # sshpass -p "scope" ssh $gnb "sed -i '13s/.*/  \"slice-users\": \"{0: [8, 9, 10], 1: [2, 3, 4], 2:[5, 6, 7, 11]}\",/' /root/radio_api/radio_rb_data.conf"
    user_tuple="3 3 3"
    sshpass -p "scope" ssh $gnb "
      sed -i '8s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \
      sed -i '9s/::.*/::0/' /root/radio_code/scope_config/slicing/ue_imsi_slice.txt && \

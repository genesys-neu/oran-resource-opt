...
user_tuples[8]="1 3 5"
user_tuples[9]="0 2 2"
user_tuples[10]="1 2 1"
user_tuples[11]="0 1 2"
user_tuples[12]="2 3 4"
user_tuples[13]="1 2 5"
...
traffic_gen() {
    ...
    for entry in $slice_mapping; do
        ...
        # Determine the specific file to use based on slice type and count
        local random_file=""
        if [[ $slice_type == "mmtc" ]]; then
            random_file=$(ls "./raw" | grep -i "$slice_type" | sort | sed -n "${mmtc_count}p")
            ((mmtc_count++))
        elif [[ $slice_type == "urllc" ]]; then
            random_file=$(ls "./raw" | grep -i "$slice_type" | sort | sed -n "${urllc_count}p")
            ((urllc_count++))
        elif [[ $slice_type == "embb" ]]; then
            random_file=$(ls "./raw" | grep -i "$slice_type" | sort | sed -n "${embb_count}p")
            ((embb_count++))
        fi

        # Calculate IP based on UE ID
        local ip=$((ue_id + 1))

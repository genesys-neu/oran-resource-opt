...
def get_recursive_size(obj):
    """Recursively calculate the memory size of an object, including nested elements."""
    seen_ids = set()
    def _sizeof(o):
        if id(o) in seen_ids:
            return 0  # Avoid double-counting for objects already seen
        seen_ids.add(id(o))
...
def update_prbs_and_remove_stale_ues(ue_data, max_stale_steps):
    """
        ...
        if len(data_sck) <= 0:
            ...
            for line in unique_lines:
                ...
                if curr_timestamp > truncated_last_timestamp:
                    ...
                    if len(ue['inference_kpi']) == slice_len:
                        # Measure memory usage of the ue dictionary
                        ue_memory = get_recursive_size(ue)
                        logging.info(f"Total memory usage of UE data (imsi={imsi}): {ue_memory} bytes")
                        ...
                        logging.info(f"Inference completed in {inference_time:.6f} seconds")
                    ...
                    if count_pkl > 31:
                        ...
                        update_prbs_and_remove_stale_ues(ue_data, max_stale_steps)

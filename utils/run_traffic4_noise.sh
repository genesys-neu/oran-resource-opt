...
#sshpass -p "sunflower" ssh genesys-075 "cd /root/utils && sh uhd_tx_tone.sh" &
sleep 5 # let the tx process start
int_PID=`sshpass -p "sunflower" ssh $interferer "pgrep tx_waveforms"`
#int2_PID=`sshpass -p "sunflower" ssh genesys-075 "pgrep tx_waveforms"`
...
#sshpass -p "sunflower" ssh genesys-075 "kill -INT ${int2_PID}"
